...
# new but no longer used 
def group(ds, interval='week'):
    """
    Takes an xarray dataset containing a vegetation index 
    variable, groups and reduces values based on a specified 
    temporal group interval. The available group options are  
    dayofyear, week or month. The resulting dataset contains 
    the new grouped veg_index variable with time values 
    interpolated between 1980-01-01 to 1980-12-31 using the
    the number of dates that are produced via the group by.
    ...
    interval: str
        The time interval which to reduce the dataset to. 
        Available intervals include dayofyear, week and month.
    ...
    if not isinstance(ds, xr.Dataset):
        ...
        raise ValueError('No veg_fx variable in dataset.')
        
    # check interval
    if interval not in ['dayofyear', 'week', 'month']:
        raise ValueError('Interval must be dayofyear, week or month.')
        
    # group based on interval, rename coord back to time
    ds = ds.groupby('time' + '.' + interval).median('time')
    ds = ds.rename({interval: 'time'})
    
    # get the num of periods from grouping (could be 51, 52, 53)
    periods = len(ds['time'])
    ...
    dates = pd.date_range(start='1980-01-01', 
                          end='1980-12-31', 
                          periods=periods)
    ...
    # notify and return
    print('Grouped data successfully.')
    return ds
