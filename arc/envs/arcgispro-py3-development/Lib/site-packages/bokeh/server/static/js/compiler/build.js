...
async function build(base_dir, bokehjs_dir, base_setup) {
    ...
    const styles_dir = path_1.join(base_dir, "styles");
    const css_dir = path_1.join(dist_dir, "css");
    print("Compiling styles");
    if (!await styles_1.compile_styles(styles_dir, css_dir))
        success = false;
    styles_1.wrap_css_modules(css_dir, lib_dir, dts_dir);
    ...
    print("All done.");
    return !compiler_1.is_failed(tsoutput) && success;
}
exports.build = build;
