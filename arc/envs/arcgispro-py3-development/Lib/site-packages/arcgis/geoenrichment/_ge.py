    ...
    **Argument**           **Description**
    ...
    def __init__(self, gis, url=None, product='bao', language_code=None):
        ...
        if product is None:
            ...
            raise ValueError('Invalid product, must be: %s' % ['bao',
                                                               'ca',
                                                               'busanalystonline',
                                                               'communityanalyst'])
    ...
    def create_report(self,
        ...
        **Argument**           **Description**
        ...
        if options is not None:
            params['studyAreasOptions'] = options
        if self._gis._portal.is_arcgisonline and \
    ...
    def standard_geography_query(self,
        ...
        **Argument**               **Description**
        ...
        if not return_sub_geography is None and \
        ...
        if not out_sr is None and \
           isinstance(out_sr, int):
            params['outSR'] = out_sr
        if not return_geometry is None and \
           isinstance(return_geometry, bool):
            params['returnGeometry']  = return_geometry
        if not return_centroids is None and \
           isinstance(return_centroids, bool):
            params['returnCentroids'] = return_centroids
        if not generalization_level is None and \
           isinstance(generalization_level, int):
            params['generalizationLevel'] = generalization_level
        if not use_fuzzy_search is None and \
