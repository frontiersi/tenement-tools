...
try:
    ...
    from .._utils.text_data import save_data_in_model_metrics_html, TextDataObject, copy_metrics
    from ._arcgis_transformer import ArcGISTransformer, ModelBackbone, infer_model_type, transformer_seq_length
...
class TransformerForEntityRecognition(ArcGISTransformer):
    ...
    def __init__(self, architecture, pretrained_model_name, config=None, pretrained_model_path=None,
        ...
        super().__init__(architecture, pretrained_model_name, config, pretrained_model_path, task="ner")
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, labels=None):
        if labels is not None:
            output = self._transformer(input_ids=input_ids, attention_mask=attention_mask,
            ...
            output = self._transformer(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
        return output
    ...
    def _load_transformer(self):
        ...
        if self._pretrained_model_path and os.path.exists(self._pretrained_model_path):
            ...
            if extension == ".emd" and os.path.exists(path):
                self._transformer = AutoModelForTokenClassification.from_pretrained(path, config=self._config)
            else:
                self._transformer = AutoModelForTokenClassification.from_pretrained(self._pretrained_model_path)
        else:
            self._transformer = AutoModelForTokenClassification.\
    ...
    def precision_score(output, label, attention_mask, average="micro"):
        y_pred, y_true = TransformerForEntityRecognition.\
            get_active_predictions_labels_per_batch(output, label, attention_mask)
        return torch.tensor(round(precision_score(y_true, y_pred, average=average, zero_division=1), 4))

    @staticmethod
    def recall_score(output, label, attention_mask, average="micro"):
        y_pred, y_true = TransformerForEntityRecognition. \
            get_active_predictions_labels_per_batch(output, label, attention_mask)
        return torch.tensor(round(recall_score(y_true, y_pred, average=average, zero_division=1), 4))

    @staticmethod
    def f1_score(output, label, attention_mask, average="micro"):
        y_pred, y_true = TransformerForEntityRecognition. \
            get_active_predictions_labels_per_batch(output, label, attention_mask)
        return torch.tensor(round(f1_score(y_true, y_pred, average=average, zero_division=1), 4))
    ...
    def get_active_predictions_labels(data_loader, learner, show_progress=True):
        ...
        if show_progress:
            for i in progress_bar(range(len(data_loader))):
                ...
                batch_preds, batch_labels = TransformerForEntityRecognition.\
                    get_active_predictions_labels_per_batch(output, *batch[1])
                predictions.extend(batch_preds)
                labels.extend(batch_labels)
        else:
            results = [TransformerForEntityRecognition.get_active_predictions_labels_per_batch
                       (learner.model.forward(*batch[0]), *batch[1]) for batch in dataset]
    ...
    def generate_inference(self, text_list, device):
        ...
        encodings = tokenizer.batch_encode_plus(token_list, max_length=max_seq_length, padding=True, truncation=True,
                                                is_split_into_words=True, return_tensors="pt").to(device)

        input_ids, attention_mask = encodings.get("input_ids"), encodings.get("attention_mask")
...
class _TransformerEntityRecognizer(ArcGISModel):
    ...
    def _create_text_learner_object(self, data, backbone, pretrained_path=None, mixed_precision=False,
        ...
        recall = partial(TransformerForEntityRecognition.recall_score, average=self.stats)
        ...
        if mixed_precision:
            if model_type in ["xlnet", "mobilebert"]:
                error_message = (f"Mixed precision training is not supported for transformer model - {model_type.upper()}."
    ...
    def extract_entities(self, text_list, batch_size=4, drop=True, debug=False):
        ...
        if isinstance(text_list,  (str, bytes)):
            ...
            for item_name in item_names:
                try:
                    with open(f'{path}/{item_name}', 'r', encoding='utf-16', errors='ignore') as f:
                        text_list.append(f.read())
                    file_names.append(item_name)
                except:
                    try:
                        with open(f'{path}/{item_name}', 'r', encoding='utf-8', errors='ignore') as f:
                            text_list.append(f.read())
                        file_names.append(item_name)
                    except Exception as e:
        ...
        for i in progress_bar(range(0, len(text_list), batch_size)):
            ...
            if debug:
                ...
                batch_results, columns = self._process_results(tokens, labels, return_dataframe=False,
                            drop=drop, start_index=i, file_names=file_names[i: i + batch_size])
                results.extend(batch_results)
    ...
    def _process_results(self, tokens, predictions, return_dataframe=True, drop=False, start_index=0, file_names=[]):
        data_list = []
        ...
        for index, row in enumerate(results):
            ...
            if has_address:
                ...
                if drop is True and address_list is None:
                    continue
                if address_list is None: address_list = [""]
        ...
        df_columns = [text_tag, "Filename", address_tag, *cols] if has_address else [text_tag, "Filename", *cols]
        if return_dataframe:
    ...
    def _calculate_model_metrics(self, metric_type="all"):
        ...
        predictions, labels = TransformerForEntityRecognition. \
            get_active_predictions_labels(databunch.valid_dl, self.learn)

        if metric_type in ["precision_score", "all"]:
            metrics["precision_score"] = round(
                precision_score(labels, predictions, average=self.stats, zero_division=1), 2)

        if metric_type in ["recall_score", "all"]:
            metrics["recall_score"] = round(recall_score(labels, predictions, average=self.stats, zero_division=1), 2)

        if metric_type in ["f1_score", "all"]:
            metrics["f1_score"] = round(f1_score(labels, predictions, average=self.stats, zero_division=1), 2)
