    ...
    **Argument**            **Description**
    ...
    def fit(self, epochs=20, lr=None, one_cycle=True, early_stopping=False, checkpoint=True, **kwargs):
        ...
        **Argument**            **Description**
        ...
        other_pipes = [pipe for pipe in nlp.pipe_names if
                       pipe != 'ner']  # get names of other pipes to disable them during training
        with nlp.disable_pipes(*other_pipes):  # only train NER
            ...
            for itn in mb:
                ...
                if not lr_find: self.recorder.losses.append(
                    sum(epoch_loss) / n_iter)  # averaging loss per epoch
                ...
                if VAL_DATA:
                    ...
                    if not lr_find: self.recorder.val_loss.append(
                ...
                if lr_find:
                    ...
                    if np.mean(losses_list) > 2 * np.min(self.recorder.losses) or len(lr) == 0:
                        return
    ...
    def _save_model_characteristics(self, model_characteristics_dir):
        ...
        with io.capture_output() as captured:
            self.show_results().to_html(os.path.join(model_characteristics_dir, 'results.html'))
    ...
    def _save(self, name_or_path, zip_files=True, save_html=True, publish=False,
        ...
        if save_html:
            if self._is_empty:
                copy_metrics(self.saved_model_dir, self.model_dir, self.model_characteristics_folder)
            else:
                self._save_model_characteristics(self.model_dir.absolute() / self.model_characteristics_folder)
    ...
    def load(self, name_or_path):
        ...
        **Argument**            **Description**
        ...
        self.recorder = Recorder()
        if emd.get('metrics'): self.recorder.metrics = json.loads(emd.get('metrics'))
    ...
    def _post_process_non_address_df(self, unprocessed_df):
        ...
        for col in unprocessed_df.columns:  # converting all list columns to string
            if pd.Series(filter(lambda x: x != '', unprocessed_df[col])).apply(isinstance, args=(
            [str])).sum() == 0:  # split if this condition
                processed_df[col] = unprocessed_df[col].apply(
                    ",".join)  # join the list to string and copy to the processed df
            else:
    ...
    def _post_process_address_df(self, unprocessed_df, drop):
        ...
        for i, adds in unprocessed_df[
            address_tag].iteritems():  # duplicating rows with multiple addresses to be one row per address
        ...
        for col in processed_df.columns:  # converting all list columns to string
            if col != address_tag and pd.Series(filter(lambda x: x != '', processed_df[col])).apply(isinstance, args=(
            [str])).sum() == 0:  ## split if this condition
                processed_df[col] = processed_df[col].apply(
                    ",".join)  # join the list to strind and copy to the processed df
            else:
