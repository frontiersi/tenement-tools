...
class ChildImageClassifier:
    ...
    def updatePixels(self, tlc, shape, props, **pixelBlocks): # 8 x 224 x 224 x 3
        ...
        superres_prediction = util.pixel_classify_superres_image(self.model, batch, self.device)
        superres_prediction = batch_to_tile(superres_prediction.unsqueeze(dim=1).detach().cpu().numpy(), batch_height, batch_width)
