try:
    ...
    HAS_TORCH = False
import arcgis
from arcgis.learn import CycleGAN
try:
    import arcpy
except:
    pass
...
class ChildImageClassifier:
    ...
    def getConfiguration(self, **scalars):
        ...
        self.direction = scalars.get('direction', 'BtoA') # to convert in imagery type
    ...
    def updatePixels(self, tlc, shape, props, **pixelBlocks):
        ...
        cyclegan_prediction = util.pixel_classify_cyclegan_image(self.model, batch, self.device, self.direction, model_info=self.json_info)
        cyclegan_prediction = batch_to_tile(cyclegan_prediction.unsqueeze(dim=1).detach().cpu().numpy(), batch_height, batch_width)

        return cyclegan_prediction