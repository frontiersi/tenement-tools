...
class BahdanauAttention(nn.Module):
    ...
    def forward(self, features, hidden):
        ...
        if not self.training and B_hidden != B:
            ...
            feature_attention = feature_attention.unsqueeze(1).repeat(1, beam_size, 1, 1)
...
class DecoderAttention(nn.Module):
    def __init__(self,
        ...
        if pretrained_embeddings is not False:
            self.embed = self.load_embedding(pretrained_embeddings,
                                             vocab
                                             )
        else:
            self.embed = nn.Embedding(vocab_size, embed_size)

        self.lstm = nn.LSTMCell(2*embed_size,
                                hidden_size)
    ...
    def forward(self, features, captions_lengths, return_packed=False):
        ...
        outputs = []
        total_processed = 0
        next_words = torch.tensor([self.bos_tag] * features.shape[0]).to(features.device).long()
        for batch_size in packed[1]:
            ...
            hx, cx = self.lstm(input_batch, (hx, cx))  # batch_size, embedding_dim + attention_size
    ...
    def sample_nucleus(self, im, p, max_len):
        ...
        for i in range(max_len):
            out, hidden, attention_map = self.decode_step(current_words,
                                                          im,
                                                          hidden)
    ...
    def sample(self, features, beam_width, max_len):
        ...
        beam_search = BeamSearch(end_index=eos,
                                 max_steps=max_len,
        ...
        # Do beam search
        output = beam_search.search(start_predictions,
                                    states,
                                    partial_decode_step)
...
class EncoderDecoder(nn.Module):
    ...
    def sample(self, input_batch, beam_width=8, max_len=25):
        ...
        captions, attention_maps = self.decoder.sample(features,
...
def pack_into_batch(predictions, batches, batch_size):
    ...
    for l in batches.tolist():
        batch_of_captions.append(
            pad_tensor(predictions[total_processed:total_processed+l],
                       batch_size)
            )
        total_processed += l
...
class CorpusBLEU(Callback):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        preds, targs = post_process(last_output[0],
                                    last_target[0],
                                    last_output[1])
...
def predict_image(self,
    ...
    # get captions
    _, caption, att_vars = self.learn.model.sample(image_tensor,
                                                   beam_width=beam_width,
                                                   max_len=max_len)
    if visualize:
        fig, ax = plt.subplots()
...
def get_bleu(self, data, beam_width=5, max_len=20):
    ...
    for x_in, current_caption_gt in data.valid_dl:
        ...
        _, current_captions, _ = self.learn.model.sample(x_in,
                                                         beam_width=beam_width,
                                                         max_len=max_len)
    ...
    for pred, targ in zip(captions, captions_gts):
        ...
        targ_len += len(targ)
        for i in range(4):
            c, t = get_correct_ngrams(pred,
                                      targ,
                                      i+1,
                                      max_n=len(data.vocab.itos))
            corrects[i] += c
            counts[i] += t
