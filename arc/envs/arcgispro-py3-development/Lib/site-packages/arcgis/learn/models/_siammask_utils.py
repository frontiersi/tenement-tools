...
class SiamMask(nn.Module):
    ...
    def run(self, template, search, softmax=False):
        ...
        rpn_pred_mask = self.refine_model(feature[0], feature[1],
                                          feature[2], corr_feature,
                                          pos=None, test=torch.empty([1]))
...
class Refine(nn.Module):
    ...
    def forward(self, f0, f1, f2, corr_feature, pos=torch.empty(0), test=torch.empty(0)):
        if torch.numel(test) == 0:
            p0 = torch.nn.functional.pad(f0, [16, 16, 16, 16])[:, :, 4 * pos[0]:4 * pos[0] + 61,
                 4 * pos[1]:4 * pos[1] + 61]
...
class Custom(SiamMask):
    def __init__(self, pretrain=False, backbone="resnet50", pretrained_path="", **kwargs):
        super().__init__(**kwargs)
        self.features = ResDown(pretrain=pretrain, backbone=backbone, pretrained_path=pretrained_path)
    ...
    def track_refine(self, pos):
        pred_mask = self.refine_model(self.feature[0], self.feature[1], self.feature[2],
                                      self.corr_feature, pos=pos, test=torch.empty(0))
        return pred_mask


def build_opt_lr(trainable_params):
    optimizer = torch.optim.SGD(trainable_params, 0.001,
                                momentum=0.9,
                                weight_decay=0.0001)
...
def download_backbone(url, file_name):
    ...
    if not os.path.exists(weights_file):
        try:
            ...
            print(f"[INFO] Downloading pretrained weights for SiamMask in {weights_file}")
...
def get_learner(data=None, anchors=None):
    if data is not None:
        file_path = download_backbone(url="http://www.robots.ox.ac.uk/~qwang/resnet.model", file_name="resnet.model")
        model = Custom(anchors=anchors, pretrain=True, backbone="resnet50", pretrained_path=file_path)
        model = model.cuda()
...
def siammask_loss(outputs, *args):
    rpn_cls_loss, rpn_loc_loss, rpn_mask_loss = torch.mean(outputs['losses'][0]), torch.mean(
