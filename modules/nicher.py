...
def random_sample_absences(df_abse, num_to_sample=500):
    """
    Takes a pandas dataframe with x, y columns and various
    pseudoabsence points and randomly samples them down
    to a user-defined number.
    
    Parameters
    ----------
    df_abse: pandas dataframe
        A  pandas dataframe with x and y columns.
    num_to_sample : int
        Number of rows to sample the input dataframe
        down to.
    ...
    # check if pandas is valid 
    if df_abse is None:
        raise ValueError('Did not provide dataframe.')
    elif 'x' not in df_abse or 'y' not in df_abse:
        raise ValueError('No x or y column in dataframe.')
    elif len(df_abse) == 0:
        raise ValueError('No data in dataframe.')    
    
    # check num to sample is valid
    if num_to_sample < 1:
        raise ValueError('Random sample size must be greater than 0.')  
    
    # correct sample num if larger than dataframe
    if len(df_abse) < num_to_sample:
        num_to_sample = len(df_abse)
        
    # random sample
    df_abse = df_abse.sample(num_to_sample)
...
def generate_correlation_matrix(df_records):
    """
    Calculate a pearson correlation matrix and present correlation pairs. 
    This version does not produce a graphic, only text pearson correlation
    results. It is also assumed categorical variables are not included. See 
    superceded folder for code on matplotlib graph.
    ...
    ----------
    result : str
        A text representation of the correlation plot.
    """
    
    # set up pandas max row on print
    pd.set_option('display.max_rows', None)

    # do various checks
    if not isinstance(df_records, pd.DataFrame):
        raise TypeError('Dataframe is not a pandas dataframe type.')
    elif len(df_records) == 0:
        raise ValueError('Dataframe has no data.')
    elif len(df_records.columns) == 0:
        raise ValueError('Dataframe has no columns.')
    elif 'pres_abse' not in df_records:
         raise ValueError('No pres_abse column exists in dataframe.')

    # select presence records only and drop column
    df_pres = df_records.loc[df_records['pres_abse'] == 1]
    df_pres = df_pres.drop(columns='pres_abse')

    # check if any rows came back, if so, drop pres_abse column
    if len(df_pres) == 0:
        raise ValueError('No presence records exist in dataframe.')       
    ...
    try:
        ...
        # check if correlation pairs exist
        if len(cp) > 0:
            ...
            # set up pearson correlation guide 
            message += '\n'
            message += 'General rule of thumb:\n'
            message += '< 0.6 = No collinearity.\n'
            message += '0.6 - 0.8 = Moderate collinearity.\n'
            message += '> 0.8 = Strong collinearity. Consider removing this variable.\n'
...
def generate_vif_scores(df_records):
    """
    ...
    try:
        ...
        for col in df_pres.columns:
            ...
            y = np.array(df_pres[col])
            x = np.array(df_pres.drop(columns=col, errors='ignore'))
        ...
        if len(df_vif_scores) > 0:
            ...
            message = '\n'
            message += '- ' * 30
            message += '\nVariance Inflation Factor Scores (VIFs)\n'
            message += '- ' * 30

            # set up vif guide 
            message += '\n'
            message += 'General rule of thumb:\n'
            message += '1 = No multicollinearity.\n'
            message += '1 - 5 = Moderate multicollinearity.\n'
            message += '> 5 = High multicollinearity.\n'
            message += '> 10 = This predictor should be removed from the model.\n'
                
            # set up raw result
            message += '\n'
            message += df_vif_scores.sort_values(by='VIF Score', ascending=True).to_string()
            message += '\n'
...
def perform_sdm(ds, df, test_ratio=0.25, options=None):
    """
    Generate a species distribution model (SDM) for given estimator 
    and provided species occurrence points. Numerous parameters can 
    be set to get more out of your data - see parameters below. 
    General approach based on based on RSGISLib SDM library 
    (https://www.rsgislib.org/rsgislib_sdm.html).
    ...
    options : dict
        Modify the estimator parameters by adding sklearn random forest
        classifier options.

    Returns
    ----------
    df_result : xarray dataset
        A dataset with one variable (sdm predictions).
    """

    # check datasets
    if not isinstance(ds, xr.Dataset):
        raise TypeError('Provided dataset is not an xarray dataset type.')
    elif not isinstance(df, pd.DataFrame):
        raise TypeError('Presence/absence data not a pandas dataframe.')
    ...
    try:
        # split data in to train/test sets, always keep 1, 0 proprtions same
        df_train, df_test = train_test_split(df,
                                             test_size=test_ratio,
                                             shuffle=True,
                                             stratify=df['pres_abse'])

        # get training response (y) and predictors (x) as numpy arrays
        np_train_y = df_train['pres_abse'].values
        np_train_x = df_train.drop(columns='pres_abse').values

        # now same for testing response (y) and predictors (x) 
        np_test_y = df_test['pres_abse'].values
        np_test_x = df_test.drop(columns='pres_abse').values
    ...
    try:
        ...
        # generate estimator (random forest)
        estimator = rt(**options)

        # fit model, predict probs, subset pres probs only
        estimator.fit(X=np_train_x, y=np_train_y)
        np_prob_y = estimator.predict_proba(np_test_x)
        np_prob_y = np_prob_y[:, -1]

        # create nan mask and mask out nan(s) - classifier doesnt like nans
        da_mask = xr.where(ds.to_array().isnull(), 0, 1)
        da_mask = da_mask.min('variable')
        ds = ds.where(da_mask != 0, 0)
        ...
        # predict absence, presence probs for dataset values
        _, np_probs_p = estimator.predict_proba(das).T

        # recreate xr data array of float32 values
        da = xr.DataArray(np_probs_p.reshape(len(y), len(x)), 
                          coords={'x': x, 'y': y}, 
                          dims=['y', 'x']).astype('float32')

        # convert to dataset and reset nan pixels to nan
        ds_result = da.to_dataset(name='result')
        ds_result = ds_result.where(da_mask != 0, np.nan)

    except Exception as e:
        raise ValueError(e)        

    try:
        # calculate mean model training accuracy with 5 kfolds
        message = generate_model_accuracy(estimator=estimator,
                                          np_train_x=np_train_x,
                                          np_train_y=np_train_y,
                                          k=5)

        # calculate mean variable importance and add to above
        message += generate_feature_importance(estimator=estimator,
                                               var_names=list(ds))

        # calculate auc and pr-auc results
        message += generate_area_under_curve(np_test_y=np_test_y, 
                                             np_prob_y=np_prob_y)
    
    except:
        message = '\nCould not generate accuracy outputs.\n'
    
    return ds_result, message


def generate_model_accuracy(estimator, np_train_x, np_train_y, k=5):
    """
    Generates SDM model training accuracy and prdocues
    a string-based output for ArcGIS Pro.
    
    Parameters
    ----------
    estimator : sklearn estimator
        A sklearn estimator (random forest, extra trees).
    np_train_x : numpy array
        A array of presence/absence training values.
    np_train_y : numpy array
        A array of variable training values.
    k : int
        Number of k-folds.
    ...
    # check arrays
    if not isinstance(np_train_x, np.ndarray):
        raise TypeError('Train X is not numpy array.')
    elif not isinstance(np_train_y, np.ndarray):
        raise TypeError('Train y is not numpy array.')
        
    # check k
    if k < 1 or k > 100000:
        raise ValueError('K must be between 1 and 100000.')
    
    try:
        # generate kfolds validator
        ...
        # get summary of all k folds
        avg_acc = round(scores.mean() * 100, 2)
        avg_std = round(scores.std()  * 200, 2)
        min_acc = round(scores.min()  * 100, 2)
        max_acc = round(scores.max()  * 100, 2)
        ...
        # set up raw result
        message += '\n'
        message += 'Mean Accuracy: {}. '.format(avg_acc)
        message += 'Mean Std. Deviation: {}.'.format(avg_std)
        message += '\n'
        message += 'Minimum Accuracy: {}. '.format(min_acc)
        message += 'Maximum Accuracy: {}. '.format(max_acc)        
        message += '\n'
...
def generate_feature_importance(estimator, var_names=None):
    """
    Generates SDM mean feature importance values and 
    produces a string-based output for ArcGIS Pro.
    ...
    estimator : sklearn estimator
        A sklearn estimator (random forest, extra trees).
    var_names : list 
        List of variable names in order input into 
        estimator.
    ...
    # check if variable names provided 
    if var_names is None:
        raise ValueError('Did not provide a list of variable names.')
    elif not isinstance(var_names, list):
        raise ValueError('Variable names must be a list.')

    try:
        # set mean feature importance
        data = {
            'Variable Name': var_names,
            'Mean Importance Score': estimator.feature_importances_
        }

        # create dataframe of mean variable importance scores
        df = pd.DataFrame(data)
        
        # sort values in descending order
        df = df.sort_values(by=list(data)[1], 
                            ascending=False)

        # set up header
        message = '\n'
        message += '- ' * 30
        message += '\nMean Variable Importance Scores\n'
        message += '- ' * 30
    
        # set up mvi guide 
        message += '\n'
        message += 'General rule of thumb:\n'
        message += 'The importance of variables in \n'
        message += 'the SDM model are listed below. \n'
        message += 'The higher the value, the more \n' 
        message += 'important the variable. \n' 
...
def generate_area_under_curve(np_test_y, np_prob_y):
    """
    Generates Reciever Operator Characteristic (ROC)
    based Area Under the Curve (AUC) model accuracy 
    metric. Produces a string-based output for ArcGIS 
    Pro.
    
    Parameters
    ----------
    np_test_y : numpy array
        A numpy array of test real observations.
    np_prob_y : numpy array
    ...
    # check arrays
    if not isinstance(np_test_y, np.ndarray):
        raise TypeError('Test y is not numpy array.')
    elif not isinstance(np_prob_y, np.ndarray):
        raise TypeError('Probabilities y is not numpy array.')

    try:
        # get roc auc and round
        roc_auc = roc_auc_score(np_test_y, np_prob_y)
        roc_auc = round(roc_auc, 2)
        
        # get pr-score auc (better for many abse)
        prs_auc = average_precision_score(np_test_y, np_prob_y)
        prs_auc = round(prs_auc, 2)
        ...
        message += '\n'
        message += 'General rule of thumb:\n'
        message += 'The ROC AUC metric is indicative \n'
        message += 'of how well the current model was \n'
        message += 'able to predict the target feature. \n' 
        message += 'A value <= 0.5 is a useless model. A \n'
        message += 'value closer to 0.75 is considered moderate \n'
        message += 'and a value of 1.0 is considered perfect. \n' 
        message += 'The PR (Precision-Recall) AUC is considered \n'
        message += 'more robust when more absence points\n'
        message += 'exist than presence points.'
        message += '\n'
    
        # set up raw result
        message += '\n'
        message += 'ROC AUC: {}. '.format(roc_auc)
        message += 'PR AUC: {}'.format(prs_auc)
        message += '\n'
...
# deprecated, now done in random_sample_absence
...
# deprecated - this is more advanced than what we have in arcgis, so keep
...
# deprecated, we do this in the perform sdm now
def create_estimator(estimator_type='rf', n_estimators=100, options=None):
    """
    Create one of several types of tree-type statistical estimators. 
    This essentially sets the parameters that are used by ExtraTrees or 
    RandomForest regression to train and predict.
    ...
    options : dict
        See sklearn documentation for the dozens of options.
    Returns
    ----------
    ...
    elif not isinstance(n_estimators, int) or n_estimators < 1:
    ...
    try:
        ...
        if estimator_type == 'rf':
            estimator = rt(n_estimators=n_estimators, **options)

        elif estimator_type == 'et': 
            estimator = et(n_estimators=n_estimators, **options)

        else:
            raise ValueError('Selected estimator does not exist.')
